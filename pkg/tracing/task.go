...
type task struct {
	...
	storeErr     error              // Error from the document store, if any.
...
func runTask(ctx context.Context, task *task) {
	...
	task.mu.Lock()
	// saveTaskOutputByType may fail when the configured document store
	// rejects the write (disk full, transport error, malformed JSON for
	// TaskStorageDBJSON). The command itself succeeded, but the user only
	// learns the task ID; without surfacing the store error the Result API
	// would report StatusCompleted with empty data and no signal. Mark the
	// task failed so Result surfaces a concrete error instead.
	if task.storage == TaskStorageDB || task.storage == TaskStorageDBJSON {
		if storeErr := task.storeErr; storeErr != nil {
			task.status = StatusFailed
			task.error = fmt.Errorf("save task output: %w", storeErr)
			task.deadlineTime = time.Now().Add(10 * time.Minute)
			task.mu.Unlock()
			return
		}
...
// saveTaskOutputByType persists the command output according to the task's
